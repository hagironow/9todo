...
const RETROS = [
  {
    scope: "일간", scopeColor: T.accent, date: "2026년 5월 10일 토요일", energy: 4,
    content: "사이드 프로젝트 집중 데이. 오전에 API 연동 끝내고 오후에 포트폴리오 작업. 저녁에 아이랑 공원 산책 후 독서 30분. 균형 잡힌 하루.",
    updated: "5. 10.",
  },
  {
    scope: "주간", scopeColor: T.success, date: "2026년 20주차", energy: 3.5,
    content: "MVP 빌드 거의 완료. 사용성 테스트 2명 진행. 수요일에 번아웃 기미 — 저녁 루틴(독서)을 빼먹었더니 다음 날 집중력 떨어짐. 루틴 사수가 핵심.",
    updated: "5. 11.",
  },
  {
    scope: "일간", scopeColor: T.accent, date: "2026년 5월 9일 금요일", energy: 3,
    content: "오전 스크럼 후 스트리밍 응답 구현에 3시간 몰입. 오후 아이 하원 후 컨디션 저하. 저녁 프롬프트 튜닝은 내일로.",
    updated: "5. 9.",
  },
  {
    scope: "월간", scopeColor: T.warning, date: "2026년 4월", energy: 3,
    content: "프로젝트 4개 병렬 진행은 무리. 5월부터 AI 챗봇 + 일상 두 축으로 집중하기로. 유튜브는 격주로 전환.",
    updated: "5. 1.",
  },
  {
    scope: "일간", scopeColor: T.accent, date: "2026년 5월 8일 목요일", energy: 4,
    content: "아침 등원 후 바로 코딩 돌입. 히어로 카피 미루기 1회 — 내일 반드시. 저녁 영상 편집 인트로 완료.",
    updated: "5. 8.",
  },
];
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "18px 24px 14px", display: "flex", alignItems: "center", gap: 10, borderBottom: `1px solid ${T.border}` }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 600, color: T.fg }}>회고</span>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        {RETROS.map((retro, i) => (
          <div key={i} style={{
            ...
            {/* Updated date */}
            <span style={{ fontSize: 10, color: T.mutedFg }}>{retro.updated} 수정</span>
          </div>
        ))}
      </div>
    </div>
...
/* ══════════════════════════════════════
   ...
   ══════════════════════════════════════ */
const PROJECTS = [
  { name: "AI 챗봇 앱", color: "#60A5FA", total: 24, completed: 18 },
  { name: "일상/육아", color: "#34D399", total: 14, completed: 12 },
  { name: "포트폴리오", color: "#A78BFA", total: 8, completed: 5 },
  { name: "독서", color: "#F472B6", total: 7, completed: 6 },
  { name: "유튜브 채널", color: "#FBBF24", total: 5, completed: 2 },
];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ borderRadius: 12, border: `1px solid ${T.border}`, backgroundColor: T.card, padding: "48px 40px 48px 80px", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, marginTop: 20 }}>
          <svg width="160" height="160" viewBox="0 0 100 100">
            ...
            <text x="50" y="59" textAnchor="middle" style={{ fill: T.mutedFg, fontSize: 7 }}>태스크</text>
          </svg>
      ...
      <div style={{ borderRadius: 12, border: `1px solid ${T.border}`, backgroundColor: T.card, padding: "40px 40px 40px 80px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: T.fg }}>5월 활동 히트맵</span>
          <span style={{ fontSize: 11, fontWeight: 600, color: T.success }}>10일 연속</span>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        <span style={{ fontSize: 10, color: T.fgDim }}>43/58 완료 <span style={{ color: "rgba(255,110,110,0.6)" }}>· 5 미완료</span></span>
      </div>
    </div>
  );
}
